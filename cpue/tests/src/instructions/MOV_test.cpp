...
TEST_CASE("MOV", "[instructions]") {
    ...
    auto insn = asm_disassemble2("\x66\xB8\x12\x00", 4); // MOV ax, 0x12
    ...
    // TODO: implement movabs
    //    insn = asm_disassemble("\x48\xB8\x88\x77\x66\x55\x44\x33\x22\x11"); // MOV rax, 0x1122334455667788
    //    CPUE_INFO("{} {}", insn.mnemonic, insn.op_str);
    //    REQUIRE(cpu.handle_insn(insn).raised() == false);
    //    REQUIRE(cpu.reg(X86_REG_RAX)->read().release_value() == 0x1122334455667788);

    insn = asm_disassemble2("\xB8\x88\x77\x66\x55", 5); // MOV eax, 0x0000000055667788
    CPUE_INFO("{} {}", insn.mnemonic, insn.op_str);
    REQUIRE(cpu.handle_insn(insn).raised() == false);
    REQUIRE(cpu.reg(X86_REG_RAX)->read().release_value() == 0x55667788);

    insn = asm_disassemble2("\xBB\xAD\xDE\x00\x00", 5); // MOV ebx, 0xdead
    CPUE_INFO("{} {}", insn.mnemonic, insn.op_str);
    REQUIRE(cpu.handle_insn(insn).raised() == false);
    REQUIRE(cpu.reg(X86_REG_RBX)->read().release_value() == 0xdead);

    insn = asm_disassemble2("\x89\xD8", 2); // MOV eax, ebx
    CPUE_INFO("{} {}", insn.mnemonic, insn.op_str);
    REQUIRE(cpu.handle_insn(insn).raised() == false);
