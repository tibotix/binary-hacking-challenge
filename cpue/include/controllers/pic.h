...
public:
    explicit PIC(CPU* cpu) : m_cpu(cpu) {
        init_mmio_registers();
        for (u8 i = 0; i < NUM_IRQ_PINS; i++) {
            m_free_pins.insert(i);
        }
    };
    PIC(PIC const&) = delete;

    std::optional<PICConnectionHandle> request_connection(i8 pin = -1) {
        std::scoped_lock _(m_mutex);
        if (m_free_pins.empty())
            return {};
        if (pin < 0)
            pin = *m_free_pins.end();
        if (m_free_pins.erase(static_cast<u8>(pin)) == 0)
            return {};
        return PICConnectionHandle(pin);
    }
