...
InterruptRaisedOr<void> MMU::check_page_structure_access_rights(PageStructureEntry auto* entry, TranslationContext const& ctx) {
    ...
    if (ctx.intention == INTENTION_LOAD_SEGMENT) {
...
InterruptRaisedOr<VirtualAddress> MMU::la_to_va(LogicalAddress const& laddr, TranslationContext const& ctx) {
    ...
    ErrorCode const error_code = [&]() -> ErrorCode {
        if (ctx.intention != TranslationIntention::INTENTION_LOAD_SEGMENT)
...
InterruptRaisedOr<GDTLDTDescriptor> MMU::segment_selector_to_descriptor(SegmentSelector selector) {
    ...
    auto [base, limit] = [&]() -> std::pair<VirtualAddress, u16> {
        switch (selector.table) {
            case 0: return {m_cpu->m_gdtr.base, m_cpu->m_gdtr.limit};
            case 1: return {m_cpu->m_ldtr.hidden.cached_descriptor.base(), m_cpu->m_ldtr.hidden.cached_descriptor.limit()};
            default: fail();
        }
    }();
    if (limit < CPUE_checked_umul<u16, u16>((selector.index + 1), 8)) {
        return m_cpu->raise_interrupt(Exceptions::GP(error_code));
    }
    TranslationContext ctx = {
        .op = MemoryOp::OP_READ,
        .width = ByteWidth::WIDTH_QWORD,
        .intention = TranslationIntention::INTENTION_LOAD_SEGMENT,
    };
    auto* descriptor1 = paddr_ptr<Descriptor>(MAY_HAVE_RAISED(va_to_pa(base + (selector.index * 8), ctx)));
    if (descriptor1->access.descriptor_type() == DescriptorType::RESERVED)
        return m_cpu->raise_interrupt(Exceptions::GP(error_code));
    TODO_NOFAIL("maybe check here for present bit");
    if (!descriptor1->access.is_expanded_descriptor()) {
        return *(GDTLDTDescriptor*)descriptor1;
    }
    // check if table has enough size for expanded descriptor
    if (limit < CPUE_checked_umul<u16, u16>((selector.index + 2), 8))
        return m_cpu->raise_interrupt(Exceptions::GP(error_code));
    // check if we have access to the next 64 bit
    auto* descriptor2 = paddr_ptr<Descriptor>(MAY_HAVE_RAISED(va_to_pa(base + (selector.index + 1) * 8, ctx)));
    // this should be always 0
    if (descriptor1->access.type_value() != 0)
        return m_cpu->raise_interrupt(Exceptions::GP(error_code));
    GDTLDTDescriptor expanded_descriptor{};
...
InterruptRaisedOr<IDTDescriptor> MMU::interrupt_vector_to_descriptor(InterruptVector vector) {
    TODO("interrupt_vector_to_descriptor");
}
